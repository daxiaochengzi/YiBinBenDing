   ...
   public class YdMedicalInsuranceService: IYdMedicalInsuranceService
    {
       ...
       /// <param name="param"></param>
       /// <returns></returns>
        public YdBaseParam GetYdOutpatientPayCardParam(GetYdOutpatientPayCardParam param)
        {
            var userBase = _serviceBasicService.GetUserBaseInfo(param.UserId);
            userBase.TransKey = param.TransKey;
            ...
            ctrXml.Append($"<pi_fyze>{param.PayAmount}</pi_fyze>");//费用总额
            ctrXml.Append($"<pi_cardid>{param.PayAmount}</pi_cardid>");//费用总额
            ctrXml.Append($"<pi_hklb>{param.InsuranceType}</pi_hklb>");//划卡类别
            ctrXml.Append($"<pi_jbr>{userBase.UserName}</pi_jbr>");//划卡类别
            ctrXml.Append("</row>");
